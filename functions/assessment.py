"""
Skills function assessment.

Please read the the instructions first (separate file). Your solutions should
go below this docstring.

PART ONE: Write your own function declarations - Part 1 questions aren't
included in the doctest.

PART TWO:

    >>> is_berry("blackberry")
    True

    >>> is_berry("durian")
    False

    >>> shipping_cost("blackberry")
    0

    >>> shipping_cost("durian")
    5

    >>> append_to_list([3, 5, 7], 2)
    [3, 5, 7, 2]

    >>> calculate_price(25, "CA")
    27.0

    >>> calculate_price(400, "NM")
    420.0

    >>> calculate_price(150, "OR", 0)
    150

    >>> calculate_price(60, "PA")
    65.0

    >>> calculate_price(38, "MA")
    40.9

    >>> calculate_price(126, "MA")
    135.3

PART THREE: Write your own function declarations - Part 3 questions aren't
included in the doctest.

"""

###############################################################################

# PART ONE

# NOTE: We haven't given you function signatures or docstrings for these, so
# you'll need to write your own. These functions also aren't included in the
# doctests above, so make sure to test them on your own.


#    (a) Write a function that takes a town name as a string and evaluates to
#        `True` if it is your hometown, and `False` otherwise.

def is_my_hometown(town):
    '''Check if a town is my hometown.

    For example: 

        >>> is_my_hometown('Princeton')
        True

        >>> is_my_hometown('pRinceton')
        True

        >>> is_my_hometown('Princeton, NJ')
        True

        >>> is_my_hometown('San Francisco')
        False

        >>> is_my_hometown('Princeton34')
        False

        >>> is_my_hometown('  princeton  nj ')
        True
    '''
    town = town.lower().strip()

    if "," in town or " " in town:
        town = town.split(",")[0]   # Cut off anything after a commma
        town = town.split()[0]      # Then cut off anything after a space

    return town == 'princeton'

#    (b) Write a function that takes a first and last name as arguments and
#        returns the concatenation of the two names in one string.

def full_name(first_name, last_name):
    '''Write a full name given first and last names.  Standardize whitespace.

    For example:

        >>> full_name('Amelia', 'Bedelia')
        'Amelia Bedelia'

        >>> full_name('Pippi  ', 'Longstocking')
        'Pippi Longstocking'

        >>> full_name('Mary Kate', 'Olsen')
        'Mary Kate Olsen'
    '''

    return first_name.strip() + " " + last_name.strip()

#    (c) Write a function that takes a home town, a first name, and a last name
#        as arguments, calls both functions from part (a) and (b) and prints
#        "Hi, 'full name here', we're from the same place!", or "Hi 'full name
#        here', where are you from?" depending on what the function from part
#        (a) evaluates to.

def compare_origins(hometown, first_name, last_name):
    '''Greet the given person by name, comparing their hometown to mine.

    For example:

    >>> compare_origins('Snowsdale', 'Daine', 'Sarrasri')
    Hi Daine Sarrasri, where are you from?

    >>> compare_origins('Princeton, NJ', 'Albert', 'Einstein')
    Hi Albert Einstein, we're from the same place!

    '''
    if is_my_hometown(hometown):
        print "Hi %s, we're from the same place!" % full_name(first_name, last_name)
    else:
        print "Hi %s, where are you from?" % full_name(first_name, last_name)


###############################################################################

# PART TWO

# 1. (a) Write a function, `is_berry()`, which takes a fruit name as a string
#        and returns a boolean if the fruit is a "strawberry", "cherry", or
#        "blackberry."


def is_berry(fruit):
    """Determines if fruit is a berry"""

    return fruit.strip().lower() in {'strawberry', 'cherry', 'blackberry'}


# (b) Write another function, shipping_cost(), which calculates shipping cost
#     by taking a fruit name as a string and calling the is_berry() function
#     within the shipping_cost() function. Your function should return 0 if
#     is_berry() == True, and 5 if is_berry() == False.

def shipping_cost(fruit):
    """Calculates shipping cost of fruit"""

    if is_berry(fruit):
        return 0
    else:       # I know I don't need the else, but it seems better!
        return 5


# 2. Make a function that takes in a number and a list of numbers. It should
#    return a new list containing the elements of the input list, along with
#    given number, which should be at the end of the new list.

def append_to_list(lst, num):
    """Creates a new list consisting of the old list with the given number
       added to the end."""

    return lst + [num]



# 3. Write a function calculate_price to calculate an item's total cost by
#    adding tax, and any fees required by state law.

#    Your function will take as parameters (in this order): the base price of
#    the item, a two-letter state abbreviation, and the tax percentage (as a
#    two-digit decimal, so, for instance, 5% will be .05). If the user does not
#    provide a tax rate it should default to 5%.

#    CA law requires stores to collect a 3% recycling fee, PA requires a $2
#    highway safety fee, and in MA, there is a commonwealth fund fee of $1 for
#    items with a base price under $100 and $3 for items $100 or more. Fees are
#    added *after* the tax is calculated.

#    Your function should return the total cost of the item, including tax and
#    fees.

def calculate_price(cost, us_state, tax_percentage = 0.05):
    '''Calculate the total cost after taxes and fees applied by state.
    State must be given in two-letter capital abbreviation format.'''

    assert tax_percentage < 1 # Sales tax should be entered as a decimal, e.g. 5% specified as 0.05.
    assert cost > 0  # First argument should be a cost greater than $0

    if us_state == 'CA':
        recycling_fee = 0.03
        tax_percentage += recycling_fee
        add_fee = 0

    elif us_state == 'PA':
        highway_safety_fee = 2
        add_fee = highway_safety_fee

    elif us_state == 'MA':
        if cost < 100:
            commonwealth_fee = 1
        else:
            commonwealth_fee = 3
        add_fee = commonwealth_fee

    else:
        add_fee = 0

    return cost * (1 + tax_percentage) + add_fee
    # I wanted to write:
    # return round(cost * (1 + tax_percentage) + add_fee, 2) 
    # to round to the nearest cent. But Oregon expected an integer and it seemed
    # too strange to handle integer answers separately.


    #Took me 40 mins Saturday night but I'm gonna look it over in the morning.



###############################################################################

# PART THREE: ADVANCED

# NOTE: We haven't given you function signatures and docstrings for these, so
# you'll need to write your own. These functions also aren't included in the
# doctests above, so make sure to test them on your own.


# 1. Make a new function that takes in a list and any number of additional
# arguments, appends them to the list, and returns the entire list. Hint: this
# isn't something we've discussed yet in class; you might need to google how to
# write a Python function that takes in an arbitrary number of arguments.

def copy_extend(lst, *args):  # I guessed *args as I've seen it before, read documentation.
    '''Take a list and any number of subsequent inputs, return a new list with the arguments appended.
    Original list is not modified.
    Will not unpack iterables.
    Designed for lists only!

    For example:
        >>> copy_extend([1,2,3], 2,4,6,8)
        [1, 2, 3, 2, 4, 6, 8]

        >>> copy_extend([], 'hello')
        ['hello']

        >>> copy_extend([0], [1])
        [0, [1]]

        >>> lst = [1, 2, 3]

        >>> copy_extend(lst, 2,4)
        [1, 2, 3, 2, 4]

        >>> lst
        [1, 2, 3]

    '''

    return lst + [argument for argument in args] # First argument must be a list!


# 2. Make a new function with a nested inner function.
# The outer function will take in a word.
# The inner function will multiply that word by 3.
# Then, the outer function will call the inner function.
# Output will be the original function argument and the result of the inner
# function.

# Example:

#>>> outer("Balloonicorn")
#('Balloonicorn', 'BalloonicornBalloonicornBalloonicorn')

def string_with_triple(word):
    '''Return a tuple consisting of the word, then the word concatenated thrice.

    For example:

    >>> string_with_triple('Balloonicorn')
    ('Balloonicorn', 'BalloonicornBalloonicornBalloonicorn')

    >>> string_with_triple('hello ')
    ('hello ', 'hello hello hello ')
    '''

    def thrice(word):  # The doctests aren't testing this one! :(
        '''Take a string, return it thrice.

        For example:

        >>> thrice('abracad')
        'abracadabracadabracad'

        >>> thrice('6')
        '666'

        '''

        return word + word + word  # It's prettier than word * 3

    return (word, thrice(word))

###############################################################################




# END OF ASSESSMENT: You can ignore everything below.

if __name__ == "__main__":
    import doctest

    print
    result = doctest.testmod()
    if not result.failed:
        print "ALL TESTS PASSED. GOOD WORK!"
    print

...
def smallest_int(numbers):
    ...
    **DO NOT USE** the built-in function `min()`!
    ...
    minimum = numbers[0]    #Cannot start with None or rest will not work
...
def largest_int(numbers):
    ...
    **DO NOT USE** the built-in function `max()`!
    ...
    maximum = None      # None is smaller than any real number
    for number in numbers:
        if number > maximum:
            maximum = number
    return maximum
...
def duplicates(items):
    ...
    for element in items:
        if items.count(element) > 1 and element not in duplicates:
...
def find_letter_indices(words, letter):
    ...
    **DO NOT** use the `list.index()` method.
    ...
    return [letter_index(word, letter) for word in words]
